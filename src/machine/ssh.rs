...
pub(super) fn ssh_command(destination: &str, batch: bool) -> Command {
    ...
    if batch {
        // Batch mode never prompts, and its output is captured, so it needs
        // no console of its own — without this a detached server flashes one
        // on Windows for every query.
