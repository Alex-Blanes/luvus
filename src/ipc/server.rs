        ...
        if app.relaunch_requested {
            ...
            match relaunch_plan(&clients, foreground) {
                ...
                Some(plan) => {
                    ...
                    // The snapshot is written by the final save after this loop,
                    // which runs before the server lets go of its endpoints —
                    // and the successor waits for exactly that.
...
fn ends_client_writer(message: &ServerMessage) -> bool {
    matches!(
        message,
        ServerMessage::Detach | ServerMessage::ServerShutdown { .. } | ServerMessage::Relaunch
    )
}
