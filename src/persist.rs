...
struct SessionEvidence {
    ...
    /// This fork's `restart.log` lines so far, one per pane decided here; the
    /// off-thread half appends its own and writes them (see [`log_resolution`]).
...
fn capture_session_evidence(app: &App) -> SessionEvidence {
    ...
    let agent_names = unbound
        .keys()
        .map(|(agent, _)| agent)
        .filter(|agent| app.manifests.is_agent(agent))
        .cloned()
        .collect();
    SessionEvidence {
        out,
...
fn resolve_pane_sessions(evidence: SessionEvidence) -> HashMap<PaneId, Option<(String, String)>> {
    ...
    for ((agent, cwd), pane_ids) in unbound {
        ...
        if pane_ids.len() == 1 && sessions.len() == 1 {
            ...
            let agent_pane = agent_names.contains(&agent);
            for id in pane_ids {
                if agent_pane {
                    resolved.push((
                        id.0,
                        format!(
                            "{}:{agent}=none/ambiguous(panes={panes},sessions={found})",
                            id.0
                        ),
                    ));
                }
...
mod restart_log_tests {
    ...
    fn the_restart_log_records_a_resolution_once_per_change() {
        ...
        let saves: Vec<&str> = text
            .lines()
            .filter(|l| l.contains("save resolved"))
            .collect();
