...
pub struct App {
    ...
    /// Each dock's slot, as laid out last frame (this fork). Its first row is the
    /// header — click that to fold/unfold the dock. Header controls (the `+`
    /// button, the AGENTS filter) are hit-tested first, so they keep the cells
    /// they occupy.
    pub dock_slots_geom: Vec<(Side, DockKind, Rect)>,
    /// Docks folded to their header row, by dock id (this fork), mirrored from
    /// `config.collapsed_docks`. Held here rather than on `Sidebars`, which is
    /// rebuilt from each client's layout and would drop it.
    pub collapsed_docks: HashSet<String>,
...
impl App {
    pub fn new(cols: u16, rows: u16, app_tx: Sender<AppEvent>) -> Result<App> {
        ...
        let collapsed_docks: HashSet<String> = config.collapsed_docks.iter().cloned().collect();
    ...
    /// Whether a dock is folded to its header row (this fork).
    pub fn dock_is_folded(&self, kind: &DockKind) -> bool {
        self.collapsed_docks.contains(kind.id())
    }

    /// Fold or unfold a dock and persist the choice (this fork). The saved list
    /// is sorted so an unchanged fold never rewrites the config.
    pub fn toggle_dock_fold(&mut self, kind: &DockKind) {
        let id = kind.id().to_string();
        if !self.collapsed_docks.remove(&id) {
            self.collapsed_docks.insert(id);
        }
        let mut ids: Vec<String> = self.collapsed_docks.iter().cloned().collect();
        ids.sort();
        self.config.collapsed_docks = ids;
        self.persist_config();
    }
    ...
    /// Record every live agent session's current title (this fork), so the
    /// session keeps a readable name in the AGENTS history once its pane is
    /// gone. The agent owns the name: it rewrites its title as the work moves on
    /// (Claude does it again after `/compact`), and the last one seen wins.
    pub(crate) fn remember_session_titles(&mut self) {
    ...
    pub fn update_dock_resize(&mut self, _c: u16, r: u16) {
        ...
        // This fork: a folded dock is just its header row — it has no rows to
        // trade, so a rule touching one does not move.
        let folded: Vec<bool> = state.docks.iter().map(|d| self.dock_is_folded(d)).collect();
        if folded[index] || folded[index + 1] {
            return;
        }
        let previous = state.dock_weights();
        ...
        // A folded dock renders one row, but that is not the height it should
        // get back when unfolded: keep its share in the scale of the open
        // heights now stored (their average, as `push_dock` does), rather than
        // freezing it at 1.
        if folded.iter().any(|f| *f) {
            let open: Vec<u32> = heights
                .iter()
                .zip(&folded)
                .filter(|(_, f)| !**f)
                .map(|(h, _)| u32::from(*h))
                .collect();
            let avg = (open.iter().sum::<u32>() / open.len().max(1) as u32).max(1) as u16;
            for (i, f) in folded.iter().enumerate() {
                if *f {
                    heights[i] = if previous.iter().all(|w| *w == previous[0]) {
                        avg
                    } else {
                        previous[i].max(1)
                    };
...
mod tests {
    ...
    fn sidebar_dock_cap_is_enforced() {
        ...
        let cfg = crate::config::SidebarsConfig {
            ...
            right: crate::config::SideConfig {
                ...
                width: 26,
                docks: Vec::new(),
                ..crate::config::SideConfig::right_default()
            },
            files_side: None,
        };
    ...
    fn copy_mode_counts_rows_and_moves_by_half_pages() {
        ...
        // Re-read the count instead of reusing the one taken above: the render
        // in the middle of this test resizes the PTY to the drawn pane rect, and
        // a shorter viewport pushes rows into scrollback. This fork's dock
        // chrome makes that rect a different height, so the pre-render snapshot
        // is no longer what "the newest row" means here.
        let last_row = app
            .panes
            .get(&pane)
            .expect("pane")
