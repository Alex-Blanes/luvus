...
impl App {
    ...
    /// `server.stop` this brings the foreground client back by itself, so it is
    /// the only way to load a new build from inside luvus. Reports the
    /// confirmation instead of acting while agents work.
    pub(super) fn api_server_relaunch(&mut self, method: &str, p: &Value) -> DispatchResult {
        let _ = method;
        reject_api_fields(p, &[])?;
...
impl App {
    ...
    pub(in crate::app) fn apply_socket_config(
        ...
        self.collapsed_docks = next.collapsed_docks.iter().cloned().collect();
