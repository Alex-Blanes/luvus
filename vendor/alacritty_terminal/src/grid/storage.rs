...
/// Maximum number of cached row bytes outside the logical grid.
///
/// A fixed row count scales poorly with terminal width: 1,000 wide rows can
/// retain several MiB after the scrollback limit is reached. Keep the cache
/// large enough to amortize normal output while bounding its memory cost.
const MAX_CACHE_BYTES: usize = 512 * 1024;

/// Never cache more rows than the old fixed cache, including for very narrow
/// generic storage types.
const MAX_CACHE_ROWS: usize = 1_000;

#[inline]
fn cache_size<T>(columns: usize) -> usize {
    let row_bytes = mem::size_of::<Row<T>>()
        .saturating_add(columns.max(1).saturating_mul(mem::size_of::<T>()));

    MAX_CACHE_BYTES.saturating_div(row_bytes.max(1)).clamp(1, MAX_CACHE_ROWS)
}
...
impl<T> Storage<T> {
    ...
    pub fn shrink_lines(&mut self, shrinkage: usize) {
        ...
        // Free memory.
        let columns = self.inner.first().map(Row::len).unwrap_or(1);
        if self.inner.len() > self.len + cache_size::<T>(columns) {
            self.truncate();
        }
    }
    ...
    /// Drop all cached rows and release unused outer-vector capacity.
    ///
    /// This is intentionally explicit: regular terminal output keeps a small
    /// cache for throughput, while memory-reduction actions can reclaim it.
    #[inline]
        ...
        T: Default,
    {
        if self.len + additional_rows > self.inner.len() {
            ...
            let cache_size = cache_size::<T>(columns);
            let realloc_size = self.inner.len() + max(additional_rows, cache_size);
...
mod tests {
    ...
    use crate::grid::storage::{cache_size, Storage, MAX_CACHE_BYTES};
    ...
    fn grow_after_zero() {
        ...
        expected.inner.append(&mut vec![filled_row('\0'); cache_size::<char>(1)]);
    ...
    fn initialize() {
        ...
        let expected_init_size = std::cmp::max(init_size, cache_size::<char>(1));
    ...
    #[test]
    fn cache_size_is_bounded_by_bytes_for_wide_cells() {
        let columns = 258;
        let rows = cache_size::<Cell>(columns);
        let row_bytes = std::mem::size_of::<Row<Cell>>()
            + columns * std::mem::size_of::<Cell>();

        assert!(rows * row_bytes <= MAX_CACHE_BYTES);
        assert!(rows < cache_size::<Cell>(80));
    }

    #[test]
    fn compact_drops_cached_rows() {
        let mut storage = Storage::<char>::with_capacity(1, 258);
        storage.initialize(1, 258);
        assert!(storage.inner.len() > storage.len);

        storage.compact();
        assert_eq!(storage.inner.len(), storage.len);
        assert!(storage.inner.capacity() <= storage.len);
    }
