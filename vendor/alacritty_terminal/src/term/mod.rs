...
impl<T> Term<T> {
    ...
    /// Release cached scrollback rows on both screen buffers.
    ///
    /// This is intended for explicit memory-reduction actions, not normal
    /// output handling where the grid cache improves throughput.
