...
Requests and replies are newline-terminated JSON. A connection may carry one or
more ordinary request/reply pairs in order:
...
For scripts that need several operations, send NDJSON through one client with
`bohay api < requests.ndjson`; it writes one response line per request. Existing
`bohay <verb>` commands remain one-shot. `events.subscribe` is the exception:
after its acknowledgment, that connection becomes an event stream.
